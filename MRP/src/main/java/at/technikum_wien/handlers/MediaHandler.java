...
public class MediaHandler implements HttpHandler {
    ...
    private void handleSearchMedia(HttpExchange exchange, String query) throws IOException {
        try {
            ...
            Integer releaseYear = queryParams.get("releaseYear") != null ? Integer.parseInt(queryParams.get("releaseYear")) : null;
            Integer ageRestriction = queryParams.get("ageRestriction") != null ? Integer.parseInt(queryParams.get("ageRestriction")) : null;
    ...
    private void handleCreateMedia(HttpExchange exchange, String requestBody) throws IOException {
        try {
            ...
            Integer releaseYear = mediaData.get("releaseYear") != null ? Integer.parseInt(mediaData.get("releaseYear").toString()) : null;
            Integer ageRestriction = mediaData.get("ageRestriction") != null ? Integer.parseInt(mediaData.get("ageRestriction").toString()) : null;

            @SuppressWarnings("unchecked") List<String> genres = (List<String>) mediaData.get("genres");
    ...
    private void handleUpdateMedia(HttpExchange exchange, int mediaId, String requestBody) throws IOException {
        try {
            ...
            if (mediaData.containsKey("genres")) {
                @SuppressWarnings("unchecked") List<String> newGenres = (List<String>) mediaData.get("genres");
    ...
    private void handleRateMedia(HttpExchange exchange, String path, String requestBody) throws IOException {
        try {
            ...
            Map<String, Object> ratingData = JsonUtil.parseJsonToMap(requestBody);
            ...
            var rating = ratingService.rateMedia(mediaId, userId, stars, comment);
            String response = "{" + "\"message\": \"Rating created successfully\"," + "\"rating\": {" + "\"id\": " + rating.getId() + "," + "\"stars\": " + rating.getStars() + "," + "\"comment\": \"" + (rating.getComment() != null ? rating.getComment() : "") + "\"," + "\"confirmed\": " + rating.getConfirmed() + "}" + "}";
            sendResponse(exchange, 201, response);
    ...
    private int extractIdFromPath(String path) {
        ...
        if (path.endsWith("/rate") || path.endsWith("/favorite")) {
            ...
            //for /api/media/1, get the last part
            return Integer.parseInt(parts[parts.length - 1]);
        }
    }

    private Map<String, String> parseQueryString(String query) {
        return Arrays.stream(query.split("&")).map(param -> param.split("=")).collect(Collectors.toMap(pair -> pair[0], pair -> pair.length > 1 ? pair[1] : ""));
    }
